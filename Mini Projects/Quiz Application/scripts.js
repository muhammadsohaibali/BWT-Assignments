...
const scq = () => { j('cq', 'flex'), j('quiz-answers', 'none'), j('quiz-list', 'none') };
const slq = () => {
    j('cq', 'none'),
        j('quiz-list', 'flex'),
        j('quiz-answers', 'none'),
...
const sq = (qu) => {
    j('cq', 'none'), j('quiz-list', 'none'), j('quiz-answers', 'flex');
    ...
    cu.forEach(m => {
        ...
        d.className = 'question-box';
        d.innerHTML = `
            <span class="question">${cu.indexOf(m) + 1}. ${m.question}</span>
            <div class="answers" style="display: flex; flex-direction: column">
        `;
        qbx.append(d);
        m.answer.forEach(n => {
            let f = document.createElement('label');
            f.innerHTML = `<input type="radio" name="${m.question}"/> ${n}`;
            d.append(f);
        });
    });
}
